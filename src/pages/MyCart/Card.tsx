...
export default function Card({ id, quantity }: Readonly<Omit<Cart, 'price'>>) {
  ...
  return (
    <li className="w-full flex p-3 bg-zinc-100 rounded-lg">
      <img src={image} alt={name} className="w-24 overflow-clip aspect-square rounded-lg relative before:content-['Image_not_found'] before:absolute before:w-full before:h-full before:bg-zinc-200 before:top-0 before:left-0 before:grid before:place-items-center before:text-zinc-500 before:text-xs before:p-2.5 before:text-center" />
