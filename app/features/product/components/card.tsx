			...
			<Link to={`/product/${slug}`} className="block">
				<div className="relative overflow-hidden rounded-lg">
					<img
						...
						alt={name}
						className="object-cover w-full h-48 sm:h-56 md:h-64 relative before:content-['Image_not_found'] before:absolute before:w-full before:h-full before:bg-zinc-200 before:top-0 before:left-0 before:grid before:place-items-center before:text-zinc-500 before:text-sm transition-transform duration-300 group-hover:scale-105"
						loading="lazy"
					/>
				...
				<div className="flex flex-col mt-4 h-fit">
					...
					<h2
						className={`font-bold w-full leading-5 text-zinc-900 ${name.length > 22 && 'h-10 relative overflow-hidden'}`}
					>
						{name}
					</h2>
